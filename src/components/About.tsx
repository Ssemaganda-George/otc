      ...
      <div className="container mx-auto px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
            {/* Mission */}
            <div className="card-dark rounded-2xl p-8 shadow-card hover:shadow-blue transition-all duration-300 card-hover">
          ...
          <div className="mt-20">
            <div className="text-center mb-8">
              ...
              <p className="text-body text-dark-muted max-w-3xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                {[
                  ...
                  <div key={sector.title} className={`relative group animate-fade-in-up opacity-0`} style={{ animationDelay: `${0.2 + index * 0.1}s`, animationFillMode: 'forwards' }}>
                    <div className="card-dark border-2 border-primary/20 rounded-xl p-4 text-center shadow-card transition-all duration-300 hover:shadow-blue hover:border-primary/40 h-[120px] flex flex-col justify-center">
                      <div className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center mx-auto mb-2">
                        <sector.icon className="w-4 h-4 text-primary" />
                      </div>
                      <h5 className="font-bold text-dark-text text-sm mb-1">{sector.title}</h5>
                      <p className="text-xs text-dark-muted">{sector.desc}</p>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                {[
                  ...
                  <div key={dept.title} className={`relative group animate-fade-in-up opacity-0`} style={{ animationDelay: `${0.7 + index * 0.1}s`, animationFillMode: 'forwards' }}>
                    ...
                    <div className={`card-dark border-2 ${dept.color} rounded-xl p-4 text-center shadow-card transition-all duration-300 hover:shadow-blue hover:border-primary hover:scale-105 group-hover:animate-glow-pulse`}>
                      <div className="text-2xl mb-2">{dept.icon}</div>
                      <h5 className="font-bold text-dark-text text-sm mb-1">{dept.title}</h5>
                      <p className="text-xs text-dark-muted">{dept.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="mt-24 bg-dark-card/50 rounded-3xl p-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
              {[
                ...
                <div key={pillar.title} className={`animate-fade-in-up opacity-0`} style={{ animationDelay: `${0.5 + index * 0.1}s`, animationFillMode: 'forwards' }}>
                  <div className="card-dark border-2 border-primary/20 rounded-xl p-6 shadow-card transition-all duration-300 hover:shadow-blue hover:border-primary/40 h-[380px] flex flex-col">
                    <h5 className="text-lg font-semibold text-primary mb-6 pb-3 border-b border-primary/30">{pillar.title}</h5>
                    <div className="flex-1">
                      <ul className="space-y-3">
                        {pillar.items.map((item, itemIndex) => (
                          <li key={itemIndex} className="flex items-start space-x-3">
                            <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0"></div>
                            <span className="text-sm text-dark-text leading-relaxed">{item}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
            ...
            <div className="card-dark border-2 border-primary/20 rounded-2xl p-8 text-center shadow-card">
              ...
              <p className="text-lg text-dark-text leading-relaxed max-w-4xl mx-auto">
