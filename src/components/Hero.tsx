...
export function Hero() {
	...
	return (
		<section
			id="home"
			className="relative h-screen flex items-center justify-center overflow-hidden pt-32 pb-12"
		>
				...
				<div className="max-w-5xl mx-auto space-y-6">
					...
					<div className="relative h-40 md:h-32 lg:h-28 mb-8 overflow-hidden">
						{heroSlides.map((slide, index) => (
							<p
								key={index}
								className={`absolute inset-0 text-xl md:text-2xl max-w-4xl mx-auto text-justify sm:text-center text-white drop-shadow-md bg-gradient-to-br from-blue-900/90 via-blue-800/90 to-blue-950/90 rounded-xl px-6 py-4 transition-all duration-1000 ease-in-out flex items-center justify-center leading-relaxed ${
									index === current
										? "translate-x-0 opacity-100"
										: index === (current + 1) % heroSlides.length
										? "translate-x-full opacity-0"
					...
					{/* Stats Row */}
					<div className="grid grid-cols-3 gap-4 md:gap-6 mb-8 animate-fade-in-up">
						<div className="flex flex-col items-center space-y-1 group">
							...
							<div className="text-lg md:text-xl font-bold text-golden group-hover:text-white transition-colors">
								5+
							</div>
